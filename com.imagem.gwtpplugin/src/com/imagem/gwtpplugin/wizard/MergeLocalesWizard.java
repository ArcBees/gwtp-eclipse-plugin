...
public class MergeLocalesWizard extends Wizard implements INewWizard {
	...
	private boolean deprecatedCommentIssued = false;
	private boolean translateCommentIssued = false;
	private boolean confirmCommentIssued = false;
	...
	/**
	 * Takes every property file in extrasDir and merge them to 
	 * the default locale file in resourcesDir. Then it merges 
	 * this resource file with every other non-default local 
	...
	protected boolean finish(IProgressMonitor monitor) {
		...
		try {
			...
			file.setContents(new ByteArrayInputStream(defaultLocaleProperties.toString().getBytes()), false, true, null);
			...
			for(IResource resourceResource : resourceResources) {
				...
				if(!findLocale(resourceFile).isEmpty()) {
					...
					file.setContents(new ByteArrayInputStream(otherLocaleProperties.toString().getBytes()), false, true, null);
				}
			}
	...
	protected List<IResource> enumeratePropertyFiles(IContainer dir) throws CoreException {
		...
		for(IResource resource : resources) {
			if(resource.getName().endsWith(".properties"))
				propertyFiles.add(new File(resource.getLocationURI()));
		}
	 ...
	 * 
	 * @param file The file from which to extract the locale.
	...
	public class Property {
		...
		public String stripToDoComments() {
			...
			for(String commentToStrip : commentsToStrip) {
				int index = comments.indexOf(commentToStrip);
				while(index != -1) {
					comments = comments.substring(0, index) + comments.substring(index + commentToStrip.length());
					index = comments.indexOf(commentToStrip);
				}
			}
	...
	public class PropertyCollection {
		 ...
		 * @param markDeprecated true if deprecated translations should be indicated, false otherwise.
		 * @param markTranslation true means that "# TODO: TRANSLATE" and "# TODO: CONFIRM TRANSLATION" 
