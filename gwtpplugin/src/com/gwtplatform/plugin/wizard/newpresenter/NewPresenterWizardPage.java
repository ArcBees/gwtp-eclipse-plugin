...
package com.gwtplatform.plugin.wizard.newpresenter;
...
public class NewPresenterWizardPage extends NewTypeWizardPage {
  ...
  private void doStatusUpdate() {
    ...
    IStatus[] status = new IStatus[] { fContainerStatus, fPackageStatus, fTypeNameStatus,
        fRevealInParentStatus, fPlaceStatus, fGinStatus };
  ...
  private void setDefaultValues() {
    try {
      if (tokenName != null) {
        String nameTokensValue = getJavaProject().getProject().getPersistentProperty(
      ...
      if (ginjector != null && presenterModule != null) {
        String ginjectorValue = getJavaProject().getProject().getPersistentProperty(
            new QualifiedName(Activator.PLUGIN_ID, "ginjector"));
        String presenterModuleValue = getJavaProject().getProject().getPersistentProperty(
  ...
  private void createRevealInParentControls(Composite composite, int nColumns) {
    ...
    isRevealContentEvent.addSelectionListener(new SelectionListener() {
      @Override
      public void widgetSelected(SelectionEvent e) {
        contentSlot.setEnabled(isRevealContentEvent.isEnabled() ? isRevealContentEvent
            .getSelection() : false);
        browseContentSlot.setEnabled(isRevealContentEvent.isEnabled() ? isRevealContentEvent
            .getSelection() : false);
  ...
  protected void setRevealInParentEnabled(boolean enabled) {
    ...
    contentSlot.setEnabled(isRevealContentEvent.isEnabled() ? isRevealContentEvent.getSelection()
        : false);
    browseContentSlot.setEnabled(isRevealContentEvent.isEnabled() ? isRevealContentEvent
        .getSelection() : false);
  ...
  protected IStatus revealInParentChanged() {
    ...
    if (isRevealContentEvent.isEnabled() && isRevealContentEvent.getSelection()) {
      if (contentSlot.getText().isEmpty()) {
        status
            .setError("You must enter the parent's content slot when selecting RevealContentEvent");
        return status;
      }
      ...
      try {
        ...
        if (slotName.isEmpty()) {
          status
              .setError("You must enter the slot's name (fully.qualified.ParentPresenter#SlotName)");
          return status;
        }
  ...
  protected IType chooseContentSlot() {
    ...
    FilteredTypesSelectionDialog dialog = new FilteredTypesSelectionDialog(getShell(), false,
        ...
        new ContentSlotSelectionExtension());
  ...
  protected IType chooseTokenName() {
    ...
    FilteredTypesSelectionDialog dialog = new FilteredTypesSelectionDialog(getShell(), false,
        getWizard().getContainer(), scope, IJavaSearchConstants.CLASS,
        new TokenNameSelectionExtension());
  ...
  private IType chooseGatekeeper() {
    ...
    FilteredTypesSelectionDialog dialog = new FilteredTypesSelectionDialog(getShell(), false,
        getWizard().getContainer(), scope, IJavaSearchConstants.CLASS,
        new GatekeeperSelectionExtension());
  ...
  protected IType choosePresenterModule() {
    ...
    FilteredTypesSelectionDialog dialog = new FilteredTypesSelectionDialog(getShell(), false,
        getWizard().getContainer(), scope, IJavaSearchConstants.CLASS,
        new PresenterModuleSelectionExtension());
