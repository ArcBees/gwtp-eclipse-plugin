...
package com.gwtplatform.plugin.wizard.newaction;
  ...
  private void setDefaultValues() {
    try {
      if (actionSuperclass != null) {
        String actionSuperclassValue = getJavaProject().getProject().getPersistentProperty(
      ...
      if (handlerModule != null) {
        String handlerModuleValue = getJavaProject().getProject().getPersistentProperty(
    ...
    if (!pack.getElementName().isEmpty()) {
      ...
      if (val.getSeverity() == IStatus.ERROR) {
        status.setError(Messages.format(
            NewWizardMessages.NewTypeWizardPage_error_InvalidPackageName, val.getMessage()));
        return status;
